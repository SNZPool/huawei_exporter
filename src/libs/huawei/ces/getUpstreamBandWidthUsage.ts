...
export async function getUpstreamBandWidthUsage(credential: Credential, bandwidthId: string, filter: string, period: Number):Promise<string> {
  const request = new ces.ShowMetricDataRequest();
  request.namespace = "SYS.VPC";
  request.metricName = "upstream_bandwidth_usage";
